<?php
   ...
   $bookingMyComment =  $_POST['myComment'];
      ...
      try{
         ...
         if($bookingStatus == 'C') {
            $fkReaservationTimeId = $user->getBookingReservationTimeId($bookingId);
         ...
         if(!$send) throw new Exception("Something wrong. Try again, or contact our service.");
        ...
        if(!empty($result) && $send) {
         ...
         $msg = $e->getMessage();
         $connection->rollBack();
         $user->returnJSON("ERROR","Something wrong. Try again, or contact our service.");
         return;
      }